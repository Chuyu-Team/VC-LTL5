...
_LCRT_DEFINE_IAT_SYMBOL(__GetPlatformExceptionInfo);
...
_LCRT_DEFINE_IAT_SYMBOL(__current_exception);
...
#if WindowsTargetPlatformMinVersion < __MakeVersion(5, 1, 2600)
    if (OSVersion < MakeMiniVersion(5, 1))
    {
...
#if WindowsTargetPlatformMinVersion < __MakeVersion(6, 0, 6000)
    if (OSVersion < MakeMiniVersion(6, 0))
    {
...
_LCRT_DEFINE_IAT_SYMBOL(__current_exception_context);
...
extern "C" int* __cdecl __processing_throw()
{
    ...
    if (OSVersion < MakeMiniVersion(5, 1))
    {
        return &((_ptd_msvcrt_win2k*)ptd)->_ProcessingThrow;
    }
#endif
...
_LCRT_DEFINE_IAT_SYMBOL(__processing_throw);
...
_LCRT_DEFINE_IAT_SYMBOL(_SetWinRTOutOfMemoryExceptionCallback);
...
_LCRT_DEFINE_IAT_SYMBOL(_IsExceptionObjectToBeDestroyed);
...
_LCRT_DEFINE_IAT_SYMBOL(_is_exception_typeof);
