...
#if !defined(CRTDLL)
__declspec(dllimport) EXCEPTION_DISPOSITION __cdecl _except_handler4_common(
    ...
    )
{
        ...
        #error  你编译错了吧？
    #endif
    //Vista以及更高版本直接调用 _except_handler4_common 导出函数即可
...
#if defined(CRTDLL)
_LCRT_DEFINE_IAT_SYMBOL(_except_handler4_common);
#endif