...
struct _locale_data_msvcrt_winxp
{
	...
	//20
	struct __crt_lc_time_data* lc_time_intl;
};
...
static void __free_lconv_mon(lconv* plconv)
{
	...
	{
		auto pWin7lconv = (lconv_win7*)plconv;

		//0x38
		if (pWin7lconv->_W_int_curr_symbol && IsPointerInMsvcrtDll(pWin7lconv->_W_int_curr_symbol) == false /*!= __lconv_c_data._W_int_curr_symbol*/)
		{
			free(pWin7lconv->_W_int_curr_symbol);
		}

		//0x3C
		if (pWin7lconv->_W_currency_symbol && IsPointerInMsvcrtDll(pWin7lconv->_W_currency_symbol) == false /*!= __lconv_c_data._W_currency_symbol*/)
		{
			free(pWin7lconv->_W_currency_symbol);
		}

		//0x40
		if (pWin7lconv->_W_mon_decimal_point && IsPointerInMsvcrtDll(pWin7lconv->_W_mon_decimal_point) == false /*!= __lconv_c_data._W_mon_decimal_point*/)
		{
			free(pWin7lconv->_W_mon_decimal_point);
		}

		//0x44
		if (pWin7lconv->_W_mon_thousands_sep && IsPointerInMsvcrtDll(pWin7lconv->_W_mon_thousands_sep) == false /*!= __lconv_c_data._W_mon_thousands_sep*/)
		{
			free(pWin7lconv->_W_mon_thousands_sep);
		}

		//0x48
		if (pWin7lconv->_W_positive_sign && IsPointerInMsvcrtDll(pWin7lconv->_W_positive_sign) == false /*!= __lconv_c_data._W_positive_sign*/)
		{
			free(pWin7lconv->_W_positive_sign);
		}

		//0x4C
		if (pWin7lconv->_W_negative_sign && IsPointerInMsvcrtDll(pWin7lconv->_W_negative_sign) == false /*!= __lconv_c_data._W_negative_sign*/)
		{
			free(pWin7lconv->_W_negative_sign);
		}
	}
}
...
static void __free_lconv_num(lconv* plconv)
{
	...
	if (__LTL_GetOsMinVersion() >= 0x00060001)
	{
		...
		//0x30
		if (pWin7lconv->_W_decimal_point && IsPointerInMsvcrtDll(pWin7lconv->_W_decimal_point) == false/*!= __lconv_c_data._W_decimal_point*/)
		{
			free(pWin7lconv->_W_decimal_point);
		}

		//0x34
		if (pWin7lconv->_W_thousands_sep && IsPointerInMsvcrtDll(pWin7lconv->_W_thousands_sep) == false/*!= __lconv_c_data._W_thousands_sep*/)
		{
			free(pWin7lconv->_W_thousands_sep);
		}
	}
}


static void __free_lc_time(__crt_lc_time_data* lc_time)
{
	if (!lc_time)
		return;
...
EXTERN_C _locale_t __cdecl _get_current_locale(void)
{
	...
	{
		__crt_locale_data* locinfo = (__crt_locale_data*)malloc(sizeof(__crt_locale_data));
...
#error 你编译条件错误了吧？
	...
	auto ptd = (_ptd_msvcrt_win6_shared*)__vcrt_getptd();
...
{
	//如果存在 _create_locale，那么直接调用系统的 _create_locale
	if (auto p_create_locale_msvcrt = try_get__create_locale())
	{
...
)
{
	...
	if (__LTL_GetOsMinVersion() < 0x00060002)
	{
		return common_create_locale(_category, _locale);
	}
#endif
...
EXTERN_C int __cdecl _configthreadlocale(int i)
{
	...
	{
		//WinXP不支持此接口，我们直接返回_DISABLE_PER_THREAD_LOCALE即可
	...
	*/
	...
	case -1:
		...
		//msvcrt此变量不可访问，如果调用 -1 ，那么程序什么也不做。
...
__EXPAND_MSVCRT_FUN(_free_locale);

EXTERN_C void __cdecl _free_locale(
	_In_opt_ _locale_t plocinfo
)
{
	//如果目标系统存在 _free_locale，那么直接调用它，避免结构体更改导致的不兼容问题
	if (auto p_free_locale_msvcrt = try_get__free_locale())
	{
		return p_free_locale_msvcrt(plocinfo);
	}
        ...
        )
{
	...
	if (auto p_setlocale_msvcrt = try_get_setlocale())
	{
        ...
        )
{
	...
	if (auto p__wsetlocale_msvcrt = try_get__wsetlocale())
	{
